...
| `enableTor`           | `false`     | Route requests through a local SOCKS5 proxy on `127.0.0.1:9050`    |
| `rejectUnauthorized`  | `true`      | Set to `false` to allow self-signed TLS certificates (HTTPS only)  |
| `axiosOptions`        | —           | Pass-through for any additional Axios request config                |
